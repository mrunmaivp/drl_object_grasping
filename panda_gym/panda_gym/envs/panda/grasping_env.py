...
class GraspingEnv(gym.Env):
    ...
    def _get_observation_space(self):
        ...
        lower_limits = np.concatenate((lower_ee_position, lower_obj_position, lower_gripper_force, lower_tcp_ft, lower_obj_grasped))
        upper_limits = np.concatenate((upper_ee_position, upper_obj_position, upper_gripper_force, upper_tcp_ft, upper_obj_grasped))
    ...
    def _get_new_observation(self):
        ...
        print("TCP", tcp_ft)
        observation = [gripper_position, current_obj_position, current_gripper_force, tcp_ft]
        observation = np.concatenate(observation)
        return observation
    ...
    def step(self, action):
        ...
        reward = self.calculate_reward(done, is_optimal_grasp, is_object_lifted, next_observation, check_obj_position, execution_status, out_of_workspace_flag)
    ...
    def grasp_object(self, action):
        ...
        if check_obj_position:
            ...
            is_object_grasp = False if any(abs(ft_values[3:]) > FORCE_THRESHOLD) and after_gripper_width < 0.06 else True
