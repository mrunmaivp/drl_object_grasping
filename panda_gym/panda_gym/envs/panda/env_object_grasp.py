...
ORIENTATION_DIFFERENCE_FACTOR = 0.4
...
class ObjectGraspEnv(gym.Env):
    ...
    def _get_observation_space(self):
        lower_ee_position = np.array([-np.inf, -np.inf, -np.inf])
        upper_ee_position = np.array([np.inf, np.inf, np.inf])

        lower_ee_orientation = np.array([-1, -1, -1])
        upper_ee_orientaiton = np.array([1, 1, 1])

        lower_obj_position = np.full((3), -np.inf)
        upper_obj_position = np.full((3), np.inf)

        lower_obj_orientation = np.array([-1, -1, -1])
        upper_obj_orientation = np.array([1, 1, 1])

        lower_obj_distance = np.full((1), 0)
        upper_obj_distance = np.full((1), np.inf)

        lower_gripper_force = np.array([0, 0])
        upper_gripper_force = np.array([np.inf, np.inf])

        lower_tcp_ft = np.full((6), -np.inf)
        upper_tcp_ft = np.full((6), np.inf)
    ...
    def spawn_random_object(self):
        ...
        object_path_list = ["cube/model.sdf", "stone/model.sdf"]
    ...
    def reset(self):
        ...
        self.spawned_object = self.obj_position.model_names[-1]
    ...
    def step(self, action):
        ...
        ref_ft_values = self.panda_robot.get_tcp_ft()
        is_optimal_grasp, check_obj_position = self.grasp_object(action, ref_ft_values)
        ...
        object_height = next_observation[8] * 2 + 0.04
        if next_observation[2] > 0.12:
            done = True
        ...
        reward = self.calculate_reward(done, is_optimal_grasp, is_object_lifted, next_observation, check_obj_position, response.success, response.out_of_workspace, orientation_difference_value)
    ...
    def grasp_object(self, action, ref_ft_values):
        ...
        if check_obj_position:
            ...
            gripper_width = 0.05
            object_height = object_position[2] * 2 + 0.045
            ...
            print("+++++++++++++++++++++++ GRASPING +++++++++++++++++++++++", rospy.get_time())
            ft_values = self.panda_robot.get_tcp_ft()
            diff_ft = ref_ft_values[3:] - ft_values[3:]
            ...
            gripper_alignment = gripper_position[2] < object_height
            is_object_grasp = True if all(abs(ft_values[3:]) < FORCE_THRESHOLD) and gripper_alignment and all(abs(after_gripper_forces) > 1) else False
    ...
    def calculate_reward(self, done, is_optimal_grasp, is_object_lifted, next_observation, check_obj_position, execution_status, out_of_workspace_flag, orientation_difference_value):
        reward = 0
        zero_vector = np.array([0.0, 0.0, 0.0])
        torque_difference = np.linalg.norm(np.array(abs(next_observation[18:21])) - zero_vector)
        reward = (1/ torque_difference)
        ...
        object_height = object_position[2] * 2 + 0.05
        if not done:
